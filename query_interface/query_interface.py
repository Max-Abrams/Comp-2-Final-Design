...
class Query_Interface():
    def __init__(self, all_materials):
        ...
        #We used to build a lot of our strcutures in main
        #Worse, we were doing it in the CLI, which meant every time we ran a query
        #we were rebuilding our data from scratch. Which is slow. I'm going to copy and paste all of our builds here,
        #so we only have to do so once upon creating the instance.


        #First, let's call hash tables and bloom filters we might need:
        self.ht = MaterialHashTable(200)
        self.sg_ht = SpaceGroupHashTable(100)
        self.atom_bf = BloomFilter(1000, 3, [hash_fn_3, hash_fn_4, hash_fn_5, hash_fn_1, hash_fn_2])

        #Now the bsts
        self.bst_density = bst(key_extractor=lambda m: m.density)
        self.bst_moment = bst(key_extractor=lambda m: (m.moment, m.data_id))
        self.bst_energy = bst(key_extractor=lambda m: m.energy)
        self.bst_slme = bst(key_extractor=lambda m: (m.slme, m.data_id))

        #Now load everything once: 
        ...
        while True:
            ...
            #Now we can just use our pre-builts 
                ...
                if attr_choice == '1':
                    ...
                    data = [m for m in self.all_materials if m.moment is not None]
                elif attr_choice == '3':
                    sort_key = lambda m: (m.energy, m.data_id)
                    data = self.all_materials
                elif attr_choice == '4':
                    sort_key = lambda m: (m.slme, m.data_id)
                    data = [m for m in self.all_materials if m.slme is not None]
                else:
                    print("Invalid choice.")
                    continue
